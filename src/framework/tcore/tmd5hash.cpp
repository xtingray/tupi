...
QString TMD5Hash::hashData(const char* bytes, int size)
{
    /*
...
QString TMD5Hash::hash(const QString &str)
{
    return hashData(str.toLocal8Bit());
}

QString TMD5Hash::hashData(const QByteArray& bytes)
{
    return hashData(bytes.data(), bytes.size());
}

QString TMD5Hash::hashFile(QFile& file)
{
...
QString TMD5Hash::hashFile(const QString& filename)
{
    QFile file(filename);
...
void TMD5Hash::updateHash(md5Context* ctx, const char* data, long int length)
{
    unsigned int t;
...
QString TMD5Hash::finalize(md5Context& ctx)
{
    unsigned count;
    unsigned char *p;
 ...
 */
void TMD5Hash::md5Transform(unsigned int buf[4], unsigned int const in[16])
{
...
QStringList TMD5Hash::passwords(const QString &plainPass)
{
    QStringList salts;
