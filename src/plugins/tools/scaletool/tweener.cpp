...
void Tweener::init(KTGraphicsScene *scene)
{
    ...
    QList<QString> tweenList = k->scene->scene()->getTweenNames(KTItemTweener::Scale);
    if (tweenList.size() > 0) {
...
{
    KAction *translater = new KAction(QPixmap(THEME_DIR + "icons/scale_tween.png"), 
                                      tr("Scale Tween"), this);
...
void Tweener::setEditEnv()
{
    ...
    KTScene *scene = k->scene->scene();
    k->objects = scene->getItemsFromTween(k->currentTween->name(), KTItemTweener::Scale);
